...
-- Latest quarterly balance-sheet row
latest_lq AS (
  SELECT
    symbol,
    date AS latestBalanceSheetDate,
    total_assets,
    current_liabilities,
    debt,
    non_current_debt,
...
SELECT
  ...
  fy.revenue AS revenue_annual, fy.free_cash_flow AS free_cash_flow_annual,
  fy.profit_margin AS profit_margin_annual, fy.year,
  fy.eps AS eps_annual, fy.eps_diluted AS eps_diluted_annual,

  -- Latest quarterly balance sheet (all aligned)
  lq.latestBalanceSheetDate,
  lq.total_assets AS latestTotalAssets,
  lq.current_liabilities AS latestCurrentLiabilities,
  lq.debt AS latestDebt,
  lq.non_current_debt AS latestNonCurrentDebt,
  (lq.debt - lq.non_current_debt) AS latestCurrentDebt,
  lq.cash_and_equivalents AS latestCash,

  agg5.avg_net_income_5yr, agg5.avg_fcf_5yr, agg5.avg_profit_margin_5yr,
  agg5.cagr_revenue_5yr, agg5.cagr_book_5yr,
  ...
  (ttm.ttm_net_income / NULLIF(lq.total_assets,0)) AS roa,
...
app.get('/full-snapshot/:ticker', async (req, res) => {
  ...
  const tableRules = {
    ...
    "shibui.technical_indicators": { mode: "last_days", days: 20, orderDesc: true, limit: 500 },
    "shibui.valuation": { mode: "last_days", days: 20, orderDesc: true, limit: 500 },
